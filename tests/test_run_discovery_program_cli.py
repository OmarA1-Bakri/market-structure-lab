...
def test_default_cli_freezes_pg3_sixteen_hour_pilot_with_admissible_budgets() -> None:
    pilot = discovery_cli._pilot_configuration()

    assert discovery_cli.DEFAULT_DERIVED_ROOT == Path("data/exports/derived/task14-PG-000003")
    assert discovery_cli.DEFAULT_PROGRAM_ROOT == Path("data/exports/discovery-programs/PG-000003")
    assert discovery_cli.DEFAULT_TRIAL_ROOT == Path("data/exports/trials/task14-PG-000003")
    assert pilot.selection_id == "SU-000703"
    assert pilot.preregistration_id == "PG-000003"
    assert pilot.split_id == "task14-first-real-discovery-v3"
    assert pilot.run_id == "DR-000703"
    assert pilot.dataset_snapshot_id == "DS-000703"
    assert pilot.feature_publication_id == "FP-000703"
    assert pilot.event_publication_id == "EP-000703"
    assert pilot.normalizer_id == "NZ-000703"
    assert pilot.regime_assignment_contract_id == "task14-contemporaneous-regimes-v3"
    ...
    assert pilot.program_budget.budget_id == "task14-program-budget-v3"
    assert pilot.phase3_execution.maximum_rows == 960
    assert pilot.run_max_rows == 480
    assert pilot.work_budget.budget_id == "task14-run-budget-v4"
    ...
    assert pilot.started_at.isoformat() == "2026-07-22T16:00:00+00:00"
    assert pilot.completed_at.isoformat() == "2026-07-22T16:01:00+00:00"
