...
def test_change_point_prefix_is_invariant_to_future_poison() -> None:
    ...
    first = CausalChangePointDetector(config)
    first_events = tuple(first.update(*item) for item in prefix)
    poisoned = CausalChangePointDetector(config)
...
def test_phase2_structural_events_are_lifted_at_observable_cutoff(
    ...
    detector = StructuralEventDetector()
    ...
    assert event.metadata["observable_trigger_timestamp"] == "2025-01-01T00:00:00Z"
...
def test_node_test_and_traversal_use_only_prior_snapshot_zones() -> None:
    ...
    test_detector = StructuralEventDetector()
    ...
    traversal_detector = StructuralEventDetector()
